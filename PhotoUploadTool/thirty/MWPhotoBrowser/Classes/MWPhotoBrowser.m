...
- (MWZoomingScrollView *)dequeueRecycledPage {
	MWZoomingScrollView *page = [_recycledPages lastObject];
	if (page) {
...
-(void)didStartDeletingPageAtIndex:(NSUInteger)index{
    ...
    if (page) {
        ...
        if (index+1 < [self numberOfPhotos]) {
            ...
            MWZoomingScrollView *nextPage = [self pageDisplayedAtIndex:index+1];
            nextPage.frame = (CGRect){pageRect.origin.x,nextPage.frame.origin.y,nextPage.frame.size.width,nextPage.frame.size.height};
        }
        [_recycledPages addObject:page];
        [page prepareForReuse];
        [page removeFromSuperview];
        [_visiblePages removeObject:page];
        _pagingScrollView.contentSize = (CGSize){_pagingScrollView.contentSize.width - page.frame.size.width - PADDING*2,_pagingScrollView.contentSize.height};
        [_photos removeObjectAtIndex:index];
        if (_delegate && [_delegate respondsToSelector:@selector(photoBrowser:deletedPhotoAtIndex:)]) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        ...
                                                        otherButtonTitles:NSLocalizedString(@"Save", nil), NSLocalizedString(@"Copy", nil), NSLocalizedString(@"Email", nil),NSLocalizedString(@"Delete", nil), nil] autorelease];
            } else {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
